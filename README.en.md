...
Use the installer supplied by GMD:

*   **macOS**: `.dmg` (Apple Silicon & Intel)
*   **Windows**: `.msi` (Recommended) or `.exe`
*   **Linux**: `.deb` (Debian/Ubuntu), `.rpm`, or `.AppImage` (Universal)

After installation, GMD Account Manager checks, downloads, and installs GMD-signed updates from inside the app. Do not overwrite it with another product's installer because the product identity, configuration directory, and update signature will not match.
...
This GMD customer distribution retains the required acknowledgments and license information for the open-source projects listed above.
