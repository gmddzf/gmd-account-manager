...
- **官方 OAuth 账号首次导入默认加入 API 服务账号池**：OAuth 授权页现在明确显示“同步加入 API 服务”开关，并在没有历史设置时默认开启；成功后会区分已加入、原本已在池内和因账号类型或限制被跳过。重新授权已有账号不会重复入池，用户显式关闭后也会继续尊重该选择。

### 修复

- **跟随官方最新认证逻辑修复 Codex 切号后仍使用旧账号**：覆盖当前凭证前先保存官方客户端最新轮换的 Token；默认读取 `$CODEX_HOME/auth.json`，明确配置 `keyring` / `auto` 时优先读取 `Codex Auth` keychain；切号串行执行，重写 OAuth 凭据时保留无关字段并清除旧账号身份。OAuth Token 交换与刷新请求也改用相互配套的 `originator: codex_vscode` 和 `User-Agent: codex_vscode/0.146.0`。
- **修复直接切号与 API 服务接管可能误报成功或继续显示旧状态**：切号后校验实际受管账号、默认实例绑定与速度设置；启用 API 服务后校验服务进程、Codex Provider、认证文件和 `__api_service__` 绑定。若账号或 API 服务已经切换但 Codex App 启动失败，会明确报告启动失败并重新读取界面的权威状态，不再让用户误以为仍在旧账号。

## [1.3.27] - 2026-08-21

### 修复

- **修复 Codex 自动切号面板显示内部字段名和未替换占位符**：5 小时与周额度阈值现在使用本地化用户文案，账号范围说明会显示实际启用状态，刷新间隔也使用当前语言的单位，不再暴露 `primary_window`、`secondary_window` 或 `{{status}}`。

## [1.3.26] - 2026-08-21

### 新增

- **Codex 账号页新增可直接操作的自动切号面板**：可以配置总开关、5 小时与周额度阈值、额度刷新间隔、全部或指定账号范围，以及切号后启动或重启 Codex App；复用现有配置接口，打开页面不会自动启用或触发切号。
- **官方 OAuth 账号现在始终给出美元消耗状态**：有可归属的本机 API 服务记录时显示该额度窗口内的模型价格估算金额；没有记录时明确显示“本机估算 $--”，不再用假零冒充已统计金额。

### 修复

- **GMD 中转账号改为展示接口返回的额度消耗**：`subapi.gmd.ink` 直接使用服务端返回的今日实际费用；`api.gmd.ink` 优先使用服务端今日费用，否则按同一凭据当日两次余额查询的下降量累计，不再把 Token 自行换算成金额。账号卡片、仪表盘和详情页统一将“今日 Token”替换为“今日消耗额度”。
- **GMD 中转接口识别限制为精确域名**：只对 `api.gmd.ink` 与 `subapi.gmd.ink` 自动调用对应额度协议，避免相似域名或任意第三方地址被误探测。
- **客户更新发布链适配私有 GMD 仓库**：标签发布改为先完成 Windows NSIS 与 macOS Universal 签名构建，再生成指向 `subapi.gmd.ink` 的目标清单和兼容 `latest.json`，最后发布 GitHub Release，避免首次发布因缺少历史 Release 或公开 GitHub 下载地址而失败。
...
- **新增 Codex 272K 长上下文费用保护**：三个上下文快捷配置入口统一增加 `272K` 预设，写入 `model_context_window = 272000` 并在 `240000` 自动压缩；同时保留默认、完整 `1M` 和自定义模式。客户原有的 `516K` 配置会继续按自定义值保留，只有主动保存新预设才会改动。
...
- **修复旧版 Codex 自定义中转账号反复重试 5 次**：账号记录缺少 `wire_api` 时按 Responses 默认协议处理，明确写入 HTTP-only provider，避免旧的顶层 `openai_base_url` 覆盖当前账号并误启用 WebSocket。

## [1.3.22] - 2026-08-17

### 修复

- **Codex 使用 GMD 中转站时不再协商不支持的 Responses WebSocket**：对 `api.gmd.ink` 和 `subapi.gmd.ink`，当配置缺少或残留错误的 WebSocket 能力标记时自动写入 `supports_websockets = false`，改走中转站支持的 HTTP/SSE 通道，避免服务端提前关闭流后连续重试 5 次。

## [1.3.21] - 2026-08-15

### 修复

- **Codex 切号后自动兼容系统代理，无需客户手改配置**：受管配置在该项缺失时自动加入 `respect_system_proxy = true`，完整保留原有中转地址、供应商和其他功能项；默认实例及额外实例都会等待桌面端真正就绪后再结束切号流程，减少首次打开时连续“正在重新连接 5/5”的情况，并保留用户显式关闭系统代理的选择。
- **Codex K12 账号订阅状态更准确**：配额接口正常但官方不提供订阅到期日时，显示“K12 额度可用”，不再误报“未获得订阅信息”。
- **Codex 账号用量不再用假零代替未知数据**：只有请求确实通过本机 API 服务并写入账号日志后，才显示请求数、Token 和估算费用；未追踪的直接 Codex 使用会明确显示“本机未统计”。
- **GMD 中转额度协议更稳**：`subapi.gmd.ink` 与 `api.gmd.ink` 分别使用对应的额度协议，页面只展示剩余额度、已用额度、今日请求和总请求。
- **New API 美元额度口径修正**：金额只使用已换算的 billing 数据，不再把平台内部 quota 单位误当成美元。
- **一键更新发布链改用 GMD 自有地址**：签名更新包和清单可统一发布到 GMD 更新服务，客户端只接受 GMD 域名下的签名更新。
...
- **Codex 账号卡片显示本窗口从满额到现在的用量**：API 服务池里的官方账号会显示请求数、token 和账号计费（`A $`）；只有已接入本机 API 服务并完成统计的窗口才显示数值。
...
### 修复
- **Codex 添加 API Key 账号时会带入当前 GMD 中转地址**：默认选中时，账号名称和基础地址会同步使用所选中转配置，不再误用 OpenAI 地址。
...
- **仪表盘 Codex API 卡片暗色主题显示正常**：仪表盘中的 Codex API Key 迷你卡片现在使用暗色主题面板，不再继承浅色旧版渐变背景。
...
### 新增
- **新增 Claude 平台管理**：GMD 现在可以在同一个 Claude 工作区管理 Claude 与 Claude CLI 账号，并在导航、仪表盘、平台布局和悬浮卡片中统一显示为一个 Claude 平台；支持 Claude 登录、Claude Code OAuth/API Key 账号、Claude Gateway 供应商配置、账号身份与额度卡片、GMD 中转预填，以及 Claude/CLI 各自独立的实例启动流程。
...
### 新增
- **GMD 中转现在提供更完整的密钥工作区**：保存的密钥会保留最近一次查询的余额，进入页面时会自动载入第一个已保存密钥，展示用量详情，读取当前密钥可用模型列表，并可预填到 Codex 供应商设置中，但不会直接替用户创建目标账号。
...
- **模型供应商用量查询改为 Codex 与 GMD 中转共用能力**：供应商余额和用量检测现在走统一服务路径，刷新时保留缓存用量，并一致识别不支持的 usage 接口。
...
- **GMD 中转展示在暗色主题下更清晰**：中转站文案保持中性，并为 GMD 中转页面补充面板、输入框、按钮、卡片、消息和密钥行的暗色主题样式。
