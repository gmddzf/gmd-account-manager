...
function buildUrl(repo, version, fileName, assetBaseUrl) {
...
function buildPlatformEntry(assetName, signatures, repo, version, assetBaseUrl) {
  ...
  return {
    signature,
    url: buildUrl(repo, version, assetName, assetBaseUrl),
  };
}
...
function main() {
  ...
  const assetBaseUrl = args['asset-base-url'];
  ...
  const darwinAarch64Entry = buildPlatformEntry(darwinAarch64Tar, signatures, repo, version, assetBaseUrl);
  const darwinX64Entry = buildPlatformEntry(darwinX64Tar, signatures, repo, version, assetBaseUrl);
  const windowsMsiEntry = buildPlatformEntry(windowsMsi, signatures, repo, version, assetBaseUrl);
  const windowsNsisEntry = buildPlatformEntry(windowsNsis, signatures, repo, version, assetBaseUrl);
  const linuxX64AppImageEntry = buildPlatformEntry(linuxX64AppImage, signatures, repo, version, assetBaseUrl);
  const linuxArmAppImageEntry = buildPlatformEntry(linuxArmAppImage, signatures, repo, version, assetBaseUrl);
  const linuxX64DebEntry = buildPlatformEntry(linuxX64Deb, signatures, repo, version, assetBaseUrl);
  const linuxArmDebEntry = buildPlatformEntry(linuxArmDeb, signatures, repo, version, assetBaseUrl);
  const linuxX64RpmEntry = buildPlatformEntry(linuxX64Rpm, signatures, repo, version, assetBaseUrl);
  const linuxArmRpmEntry = buildPlatformEntry(linuxArmRpm, signatures, repo, version, assetBaseUrl);
