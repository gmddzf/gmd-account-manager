...
test('can build a target manifest that downloads from the GMD release host', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'cockpit-target-manifest-'));
  const assetsDir = path.join(root, 'bundle');
  const notesFile = path.join(root, 'notes.md');
  const outputDir = path.join(root, 'output');
  const assetName = 'GMD.Account.Manager_1.3.21_x64-setup.exe';
  ...
  fs.writeFileSync(notesFile, 'GMD release notes');
  ...
  const [manifestPath] = buildTargetManifests({
    ...
    outputDir,
    targets: ['windows-x86_64-nsis'],
    assetBaseUrl: 'https://subapi.gmd.ink/gmd-account-manager/releases/v1.3.21',
  });

  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  assert.equal(
    manifest.url,
    `https://subapi.gmd.ink/gmd-account-manager/releases/v1.3.21/${assetName}`,
  );
