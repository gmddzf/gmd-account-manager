...
func (s *apiKeyPriorityStateStore) reloadIfChanged() {
	...
	s.mu.RLock()
	unchanged := s.hasDataHash && dataHash == s.lastDataHash
	s.mu.RUnlock()
	if unchanged {
		return
	}
...
func buildOllamaShowResponse(model string, modifiedAt time.Time) gin.H {
	...
	return gin.H{
		"model":        model,
		"remote_model": model,
		"license":      "Proxied via GMD API Service",
...
func main() {
	...
	manifestPath := flag.String("manifest", "", "GMD sidecar manifest file")
	quotaReserveStatePath := flag.String("quota-reserve-state", "", "GMD OAuth quota reserve state file")
	quotaPoolStatePath := flag.String("quota-pool-state", "", "GMD account-pool quota state file")
	parentPID := flag.Int("parent-pid", 0, "GMD parent process id")
	flag.Parse()
