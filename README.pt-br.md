...
**Solicitações de permissão de privacidade do macOS**: após iniciar o Codex/agente a partir do GMD 账号管理, se um comando do shell executado pelo agente acessar pastas protegidas, como Área de Trabalho, Documentos, Downloads ou Fotos, o macOS poderá exibir a solicitação como "O GMD 账号管理 gostaria de acessar...". Isso ocorre porque esses comandos são processos filhos iniciados pelo GMD 账号管理, portanto, o macOS atribui a solicitação ao aplicativo host; isso não significa, por si só, que o processo principal do GMD 账号管理 esteja ativamente verificando essas pastas. Conceda acesso somente se você confiar na tarefa atual do agente e nos comandos que ela executará. Em caso de dúvida, negue a solicitação ou execute o projeto a partir de um diretório de trabalho normal primeiro.
...
Use o instalador fornecido pela GMD:

*   **macOS**: `.dmg` (Apple Silicon & Intel)
*   **Windows**: `.msi` (Recommended) or `.exe`
*   **Linux**: `.deb` (Debian/Ubuntu), `.rpm`, or `.AppImage` (Universal)

Após a instalação, o GMD 账号管理 verifica, baixa e instala atualizações assinadas pela GMD dentro do aplicativo. Não substitua o programa por um instalador de outro produto, pois a identidade, o diretório de configuração e a assinatura de atualização não serão compatíveis.
...
1.  **Correção via linha de comando** (Recomendado):
    ...
    ```bash
    sudo xattr -rd com.apple.quarantine "/Applications/GMD 账号管理.app"
    ```
...
Esta distribuição GMD para clientes mantém os agradecimentos e as informações de licença exigidos para os projetos de código aberto listados acima.
