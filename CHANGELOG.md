...
All notable changes to GMD Account Manager will be documented in this file.
...
- **Fix repeated five-attempt Codex reconnects for older custom relay accounts**: accounts without a persisted `wire_api` now use the Responses default explicitly through an HTTP-only provider, so a stale top-level `openai_base_url` cannot override the selected account and re-enable WebSockets.

## [1.3.22] - 2026-08-17

### Fixed

- **Codex GMD relay conversations no longer negotiate an unsupported Responses WebSocket**: managed `api.gmd.ink` and `subapi.gmd.ink` providers now persist `supports_websockets = false` when the capability is missing or stale, so Codex uses the relay's HTTP/SSE path instead of reconnecting five times after the server closes the stream.

## [1.3.21] - 2026-08-15

### Fixed

- **Codex account switches now honor the system proxy without manual edits**: managed profiles add `respect_system_proxy = true` when the setting is absent, preserve every existing provider and feature value, and wait for both default and additional desktop instances to become ready before the switch flow returns. This reduces the initial `reconnecting 5/5` loop while still respecting an explicit proxy opt-out.
- **Codex K12 subscription status is now accurate**: when quota works but the provider does not supply a subscription expiry, the card shows that the K12 quota is available instead of reporting missing subscription information.
- **Unknown Codex account usage is no longer rendered as zero**: request, token, and estimated-cost chips appear only after traffic is attributed through the local API Service; direct untracked Codex use is marked as not tracked locally.
- **GMD relay usage contracts are more reliable**: `subapi.gmd.ink` and `api.gmd.ink` use their matching usage contracts, and the relay page displays only remaining quota, used quota, today's requests, and total requests.
- **New API USD accounting uses the correct units**: customer-visible money now comes from normalized billing data instead of raw internal quota units.
- **One-click updates use the GMD release host**: signed updater packages and manifests can be published under the GMD update service, and the client accepts only signed GMD-hosted updates.
...
- **Codex account cards show usage from the last full window to now**: official accounts in the API Service pool display request count, tokens, and account-billed cost (`A $`) only after a local API Service window has actually been tracked.
...
- **Settings no longer shows the legacy top banner by default**.
...
### Fixed
- **Codex now fills the selected GMD relay address when adding an API Key account**: the account name and Base URL follow the selected relay instead of the OpenAI address.
...
- **Dashboard Codex API cards look correct in dark mode**: Codex API Key mini cards on the dashboard now use a dark-theme surface instead of inheriting the legacy light gradient.
...
### Added
- **Claude platform management is now available**: GMD can manage Claude and Claude CLI accounts in one workspace and show them as one Claude platform across navigation, dashboard, layout, and floating cards. It supports Claude sign-in, Claude Code OAuth/API Key accounts, Claude Gateway provider setup, quota and identity cards, GMD relay prefill, and separate Claude/CLI instance launch flows.
...
### Added
- **GMD relay now has a fuller key workspace**: saved keys can retain the last queried balance, automatically reload the first saved key when the page opens, show usage details, read the current key's available model list, and prefill Codex provider setup without directly creating the target account.
...
- **Model-provider usage querying is shared across Codex and the GMD relay**: provider balance and usage checks now use a common service path, keep cached usage visible while refreshing, and classify unsupported usage endpoints consistently.
...
- **GMD relay presentation is clearer in dark mode**: relay copy is now neutral and the GMD relay page adds dark-theme styling for panels, inputs, buttons, cards, messages, and key rows.
