...
const CODEX_CLI_LAUNCH_MODEL_KEY_PREFIX = "gmd.codex.cli.launchModel.v1.";
const DEFAULT_CODEX_CLI_MODELS = [
  "gpt-5.6-sol",
  "gpt-5.6-terra",
  "gpt-5.6-luna",
  "gpt-5.5",
...
function readCodexCliLaunchModel(accountId: string): string {
  try {
    return (
      localStorage.getItem(`${CODEX_CLI_LAUNCH_MODEL_KEY_PREFIX}${accountId}`)?.trim() || ""
    );
  } catch {
    return "";
  }
...
function persistCodexCliLaunchModel(accountId: string, model: string): void {
  try {
    const key = `${CODEX_CLI_LAUNCH_MODEL_KEY_PREFIX}${accountId}`;
...
export function CodexAccountsPage() {
  ...
  const exportFormatOptions = useMemo<SingleSelectFilterOption[]>(
    () => [
      {
        value: "cockpit_tools",
        label: t("codex.exportFormat.cockpitTools", "GMD 账号管理"),
      },
      {
        value: "sub2api",
  ...
  const codexCliModelOptions = useMemo(() => {
    const account =
      cliLaunchModal?.target === "account"
        ? accounts.find((item) => item.id === cliLaunchModal.accountId)
        : null;
    const seen = new Set<string>();
    const models: string[] = [];
    const addModel = (value?: string | null) => {
    ...
    addModel(cliLaunchModal?.model);
    account?.api_model_catalog?.forEach(addModel);
    DEFAULT_CODEX_CLI_MODELS.forEach(addModel);

    return [
      {
        value: "",
        label: t("codex.cli.launchModelCurrent", "使用 config.toml 当前模型"),
      },
      ...models.map((model) => ({ value: model, label: model })),
    ];
  }, [accounts, cliLaunchModal?.accountId, cliLaunchModal?.model, cliLaunchModal?.target, t]);
  ...
  const apiKeyFunPrefillPayloadRef = useRef<ApiKeyFunPrefillPayload | null>(null);
  ...
  const buildApiProviderPayload = useCallback(
    (
      ...
      const gmdRelayPrefill = apiKeyFunPrefillPayloadRef.current;
      const isGmdRelayProvider = Boolean(
        isApiKeyFunProviderBaseUrl(normalizedBaseUrl) ||
        (gmdRelayPrefill?.baseUrl && isSameHttpBaseUrl(gmdRelayPrefill.baseUrl, normalizedBaseUrl)),
      );
      const gmdRelayModelCatalog = isGmdRelayProvider
        ? (gmdRelayPrefill?.modelCatalog ?? undefined)
        : undefined;
      const trimmedName = customProviderName.trim();
      const customProviderDisplayName =
        trimmedName || (isGmdRelayProvider ? GMD_API_PROVIDER_NAME : undefined);
      return {
        apiProviderMode: "custom",
        apiProviderId: isGmdRelayProvider ? GMD_API_PROVIDER_ID : undefined,
        apiProviderName: customProviderDisplayName,
        apiModelCatalog: gmdRelayModelCatalog,
        apiWireApi: isGmdRelayProvider ? (gmdRelayPrefill?.wireApi ?? "responses") : undefined,
  ...
  useEffect(() => {
    if (!showAddModal) {
      ...
      if (!pendingApiKeyFunCodexPrefillRef.current) {
        apiKeyFunPrefillPayloadRef.current = null;
      }
  ...
  const openCodexCliLaunchModal = (
    ...
    const modal: CodexCliLaunchModalState = {
      ...
      workingDirError: null,
      model: readCodexCliLaunchModel(accountId),
      launchCommand: "",
      terminalCommand: "",
      runtimePrepared: false,
      preparing: false,
  ...
  const updateCodexCliLaunchModel = (model: string) => {
    if (!cliLaunchModal) return;
    persistCodexCliLaunchModel(cliLaunchModal.accountId, model);
    setCliLaunchModal((prev) =>
      prev
        ? {
            ...prev,
  ...
  useEffect(() => {
    ...
    const requestBaseUrl = request.baseUrl?.trim() ?? "";
    ...
    const sponsorTemplate =
      sponsorApiProviderTemplates.find((template) => {
        ...
        return normalizedTemplateBaseUrl === normalizedRequestBaseUrl;
      }) ?? null;

    skipManagedProviderApiKeyAutofillRef.current = true;
    apiProviderPresetExplicitlySelectedRef.current = true;
    apiKeyFunPrefillPayloadRef.current = request;
    ...
    setNewManagedProviderNameInput(
      sponsorTemplate?.name ?? request.providerName?.trim() ?? "GMD API",
    );
      ...
      if (
        ...
        try {
          const savedProvider = await upsertCodexModelProviderFromCredential({
            ...
            sourceTag:
              providerPayload.sponsorTemplate?.id ??
              apiKeyFunPrefillPayloadRef.current?.sourceTag ??
              null,
            ...
            wireApi:
              providerPayload.sponsorTemplate?.wireApi ??
              apiKeyFunPrefillPayloadRef.current?.wireApi,
            integrationType:
              providerPayload.sponsorTemplate?.integrationType ??
              apiKeyFunPrefillPayloadRef.current?.integrationType,
          });
  ...
  const renderGridCards = (items: typeof filteredAccounts, groupKey?: string) =>
    items.map((account) => {
      ...
      const isAccessTokenOnlySubscription =
        ...
        subscriptionInfo.bucket === "entitlement_only";
      const showSubscriptionRefreshAction =
        !isApiKeyAccount &&
        !isPendingOAuthAccount &&
        (subscriptionInfo.bucket === "missing" ||
      ...
      >
        ...
        >
          <div className="modal-header cockpit-api-panel-header">
            <div>
              <h2>
                {t("codex.cockpitApi.panelTitle", "API 服务面板")}
              </h2>
              <span className="cockpit-api-panel-subtitle">
    ...
    >
                              ...
                              <span className="codex-import-api-service-toggle-copy">
                                ...
                                <small>
                                  {t(
                                    ...
                                    "保存后使用当前模型列表生成 GMD 受管的 Codex 模型目录，不覆盖用户自定义目录。",
                                  )}
                                </small>
                              </span>
                              <input
                      ...
                      if (value === "cpa" && exportHasAgentIdentity) {
                        reportExportModalError(
                          t(
                            ...
                            "Agent Identity 账号不支持 cpa 格式，请使用 GMD 账号管理 或 sub2api 格式导出。",
                          ),
                        );
                        return;
                      }
      ...
      {cliLaunchModal && (
        <CodexCliLaunchDialog
          ...
          model={cliLaunchModal.model}
          modelOptions={codexCliModelOptions}
          onModelChange={updateCodexCliLaunchModel}
          terminal={selectedTerminal}
