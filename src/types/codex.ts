...
// Older builds used a fixed Cockpit relay URL. Customer builds must use the
// address stored on each account instead, so this compatibility matcher is
// intentionally disabled while the legacy function remains available.
...
export function isCodexCockpitApiBaseUrl(rawValue?: string | null): boolean {
  if (!COCKPIT_API_BASE_URL) return false;
  return (
...
function getCodexPlanBadgeLabel(account: CodexAccount): string {
  if (isCodexNewApiAccount(account)) {
    return account.plan_type?.trim() || "API";
  }
...
function isCodexEntitlementWithoutSubscriptionDate(account: CodexAccount): boolean {
  if (!account.quota) return false;
  const lastError = account.subscription_query_last_error || "";
  return /no subscription found for account/i.test(lastError);
}
...
export function getCodexSubscriptionPresentationForAccount(
  ...
  if (isCodexEntitlementWithoutSubscriptionDate(account)) {
    const plan = getCodexPlanDisplayName(account.plan_type);
    const valueText = t("codex.subscription.entitlementUsable", {
      plan,
      defaultValue: `${plan} 额度可用`,
    });
    const detailText = t("codex.subscription.entitlementUsableDetail", {
      defaultValue: "官方未提供订阅到期日，5h/Weekly 配额仍可正常刷新",
    });
    return {
      bucket: "entitlement_only",
      tone: "active",
      valueText,
      detailText,
