...
import {
  ...
  formatCodexWindowCostDisplay,
  formatCodexWindowStatsText,
  hasCodexWindowStatsSnapshot,
...
test("formats compact numbers like Sub2API", () => {
  ...
  assert.equal(
    formatCodexWindowCostDisplay({ estimatedCostUsd: 0, costAvailable: false }),
    "--",
  );
  assert.equal(
    formatCodexWindowCostDisplay({ estimatedCostUsd: 12.5, costAvailable: true }),
    "12.50",
  );
});
...
test("distinguishes a tracked zero result from missing local API statistics", () => {
  const trackedZero = {
    requestCount: 0,
    inputTokens: 0,
    cachedInputTokens: 0,
  ...
  assert.equal(hasCodexWindowStatsSnapshot(trackedZero), true);
  assert.equal(hasCodexWindowStatsSnapshot(undefined), false);
  assert.equal(hasCodexWindowStatsSnapshot(null), false);
});
