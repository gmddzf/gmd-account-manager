...
export interface CodexWindowStats {
  ...
  /** Whether estimatedCostUsd is backed by a recorded model price. */
  costAvailable?: boolean;
  /** 下游用户计费；当前 API 服务没有该口径时不展示 U。 */
  userCostUsd?: number | null;
}

/** 区分“已统计且结果为 0”和“该账号没有本机 API 服务统计”。 */
export function hasCodexWindowStatsSnapshot(
  stats?: CodexWindowStats | null,
): stats is CodexWindowStats {
  return stats != null;
}
...
export function formatCodexWindowCostDisplay(stats: Pick<CodexWindowStats, "estimatedCostUsd" | "costAvailable">): string {
  return stats.costAvailable === false
    ? "--"
    : formatCodexWindowCostAmount(stats.estimatedCostUsd);
}
