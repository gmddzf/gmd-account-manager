...
function CodexQuotaMiniRow({
  ...
  const stats = hasCodexWindowStatsSnapshot(item.windowStats)
    ? item.windowStats
    : undefined;
  const showUntrackedState =
    !stats && (item.key === "primary" || item.key === "secondary");
  return (
      ...
      <div className="codex-quota-mini-head">
        ...
        {stats ? (
          ...
          <div
            className="codex-quota-mini-right"
            title={t(
              "codex.quota.windowStatsUnavailableHint",
