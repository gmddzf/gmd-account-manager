      ...
      <div className="modal modal-lg">
        ...
        <div className="modal-body">
          ...
          {modelOptions && onModelChange && (
            <div className="form-group" data-onboarding="codex-cli-model">
              <label>{t("codex.cli.launchModel", "启动模型")}</label>
              <SingleSelectDropdown
                value={model ?? ""}
                onChange={onModelChange}
                options={modelOptions}
                disabled={busy}
                ariaLabel={t("codex.cli.launchModel", "启动模型")}
              />
              <p className="form-hint">
                {t(
                  "codex.cli.launchModelDesc",
                  "选定后会作为 --model 参数启动 Codex CLI；每个账号会记住自己的选择。",
                )}
              </p>
            </div>
          )}
