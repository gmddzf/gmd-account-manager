...
/**
 * Build a non-secret identity for local usage snapshots. Never persist the API
 * key itself: provider/key IDs are generated by the local account manager and
 * are sufficient to keep balances for different credentials separate.
 */
export function resolveCodexModelProviderUsageIdentity(input: {
  provider?: Pick<CodexModelProvider, 'id' | 'apiKeys'> | null;
  apiKey?: string | null;
  accountId?: string | null;
}): string | null {
  const normalizedKey = input.apiKey?.trim();
  if (input.provider && normalizedKey) {
    const matched = input.provider.apiKeys.find(
      (item) => item.apiKey.trim() === normalizedKey,
    );
    if (matched) {
      const revision = matched.updatedAt || matched.createdAt || 0;
      return `provider:${input.provider.id}:key:${matched.id}:rev:${revision}`;
    }
  }
