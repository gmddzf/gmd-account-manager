...
const GMD_API_PROVIDER_ID: &str = "gmd_api";
const LEGACY_APIKEY_FUN_PROVIDER_ID: &str = "apikey_fun";
const COCKPIT_API_PLAN_TYPE: &str = "Cockpit Api";
const LEGACY_NEW_API_EXCLUSIVE_PLAN_TYPE: &str = "NEW_API_EXCLUSIVE";
const GMD_API_PLAN_TYPE: &str = "GMD API";
...
fn is_new_api_account(account: &CodexAccount) -> bool {
    account
        ...
        .map(|value| {
            ...
            value.eq_ignore_ascii_case(GMD_API_PROVIDER_ID)
                || value.eq_ignore_ascii_case(LEGACY_APIKEY_FUN_PROVIDER_ID)
        ...
        || account
            ...
            .map(|value| {
                let value = value.trim();
                value.eq_ignore_ascii_case(GMD_API_PLAN_TYPE)
...
fn is_cockpit_api_base_url(raw: Option<&str>) -> bool {
    if COCKPIT_API_BASE_URL.is_empty() {
        return false;
    }
...
fn build_new_api_profile_url(account: &CodexAccount) -> Result<String, String> {
    let base_url = account
        ...
        .ok_or("GMD API 账号缺少 Base URL")?;
    let mut parsed = reqwest::Url::parse(base_url)
        .map_err(|err| format!("GMD API Base URL 无效: {}", err))?;
    if !matches!(parsed.scheme(), "http" | "https") {
        return Err("GMD API Base URL 仅支持 http/https".to_string());
    }
...
async fn fetch_new_api_quota(account: &CodexAccount) -> Result<FetchQuotaResult, String> {
    let api_key = account
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or("GMD API 账号缺少 OPENAI_API_KEY")?;
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|err| format!("请求 GMD API 额度失败: {}", err))?;
    let status = response.status();
    let body = response
        .text()
        .await
        .map_err(|err| format!("读取 GMD API 额度响应失败: {}", err))?;
    if !status.is_success() {
        return Err(format!("GMD API 额度接口返回 HTTP {}", status.as_u16()));
    }

    let root: serde_json::Value = serde_json::from_str(&body)
        .map_err(|err| format!("解析 GMD API 额度 JSON 失败: {}", err))?;
    if root.get("success").and_then(|item| item.as_bool()) == Some(false) {
        let message = root
            .get("message")
            .and_then(|item| item.as_str())
            .unwrap_or("GMD API 额度接口返回失败");
        return Err(message.to_string());
    }
    let data = root.get("data").unwrap_or(&root);
    let usage = data.get("usage").ok_or("GMD API 额度响应缺少 usage")?;
