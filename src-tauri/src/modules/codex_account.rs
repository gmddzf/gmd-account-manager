...
const GMD_API_LOGIN_PLAN_TYPE: &str = "GMD API";
...
// The customer build never hardcodes a model relay. Keep the old matcher
// disabled so saved legacy accounts can still be read without routing new
// traffic to an upstream-owned endpoint.
const CODEX_COCKPIT_API_BASE_URL: &str = "";
const CODEX_COCKPIT_API_PROVIDER_ID: &str = "cockpit_api";
const CODEX_GMD_API_PROVIDER_ID: &str = "gmd_api";
const CODEX_LEGACY_APIKEY_FUN_PROVIDER_ID: &str = "apikey_fun";
...
fn is_cockpit_api_base_url(raw: Option<&str>) -> bool {
    if CODEX_COCKPIT_API_BASE_URL.is_empty() {
        return false;
    }
...
fn apply_api_key_fields(
    ...
    let provider_name = provider_config.provider_name.as_deref();
    let is_gmd_api = provider_id
        .map(|value| {
            value.eq_ignore_ascii_case(CODEX_GMD_API_PROVIDER_ID)
                || value.eq_ignore_ascii_case(CODEX_LEGACY_APIKEY_FUN_PROVIDER_ID)
        })
        .unwrap_or(false)
        || provider_name
            .map(|value| value.trim().eq_ignore_ascii_case("gmd api"))
            .unwrap_or(false)
        || is_cockpit_api_base_url(provider_config.base_url.as_deref());
    let is_legacy_cockpit_api = provider_id
        .map(|value| value.eq_ignore_ascii_case(CODEX_COCKPIT_API_PROVIDER_ID))
        .unwrap_or(false)
        || is_cockpit_api_base_url(provider_config.base_url.as_deref());
    let plan_type = if is_gmd_api {
        GMD_API_LOGIN_PLAN_TYPE
    } else if is_legacy_cockpit_api {
    ...
    if normalize_optional_ref(account.account_name.as_deref()).is_none() {
        if is_gmd_api {
            account.account_name = Some("GMD API".to_string());
        } else if is_legacy_cockpit_api {
...
fn collect_managed_api_key_provider_ids() -> HashSet<String> {
    HashSet::from([
        ...
        CODEX_GMD_API_PROVIDER_ID.to_string(),
        CODEX_LEGACY_APIKEY_FUN_PROVIDER_ID.to_string(),
...
fn api_key_account_requires_bearer_provider_override(
    ...
    // Codex's native Responses path is the default when an older account record
    // has no `api_wire_api`.  Treat that missing value as Responses here as well;
    // otherwise a custom relay with WebSockets disabled falls through to the
    // built-in `openai` provider and silently uses a stale top-level
    // `openai_base_url`, which can bring the broken WebSocket transport back.
...
pub(crate) fn write_prepared_account_bundle_to_dir(
    ...
    // Persist the proxy feature at switch time as well as at managed launch
    // time. Customers may switch here and then open Codex directly from the
    // desktop or a terminal, bypassing the GMD launch command entirely.
...
mod tests {
    use super::{
        ...
        load_account_index, load_local_oauth_snapshot_from_official_store_with_keychain_reader,
        looks_like_sub2api_export, merge_existing_auth_file_value, now_timestamp,
...
{"id_token":"id-2","access_token":"access-2"}"#;
    ...
    #[test]
    fn auth_file_merge_preserves_custom_fields_and_drops_stale_identity() {
        ...
        assert_eq!(merged["custom_device_id"], "keep-me");
        assert!(merged.get("email").is_none());
        assert!(merged.get("access_token").is_none());
        assert!(merged.get("refreshToken").is_none());
        assert!(merged.get("session_token").is_none());
        assert_eq!(merged["tokens"]["access_token"], "new-token");
    }
    ...
    fn default_auth_store_prefers_auth_json_over_keychain() {
        ...
        let snapshot =
            load_local_oauth_snapshot_from_official_store_with_keychain_reader(&base_dir, |_| {
                Ok(Some(build_oauth_auth_file(&keychain_tokens, "acc-current")))
            })
    ...
    #[test]
    fn switch_presync_rejects_stale_official_credentials() {
        let _lock = crate::modules::test_support::env_lock()
            .lock()
            .unwrap_or_else(|err| err.into_inner());
        let env = TestEnvGuard::new("codex-switch-presync-stale-auth-test");
        let mut current = seed_oauth_account(make_codex_tokens(
            "demo@example.com",
            "acc-current",
            "org-current",
        ...
        current.token_updated_at = Some(now_timestamp() + 60);
        save_account(&current).expect("make stored credential newer than official refresh");

        let stale_tokens = make_codex_tokens(
            "demo@example.com",
            "acc-current",
            "org-current",
            "official-stale",
        ...
        write_oauth_auth_file(&env.codex_home(), &stale_tokens, "acc-current");

        assert!(!super::sync_active_official_account_before_switch()
            .expect("reject stale official account"));
        let persisted = load_account(&current.id).expect("load current account after presync");
        assert_eq!(persisted.tokens.access_token, current.tokens.access_token);
        assert_eq!(
            persisted.tokens.refresh_token.as_deref(),
            Some("rt-stored-newer")
        );
    }
...
"#,
    ...
    #[test]
    fn api_key_config_defaults_missing_wire_to_http_only_provider() {
        let base_dir = make_temp_dir("codex-api-key-config-missing-wire-test");
        // Older imported accounts may not have api_wire_api persisted.  A custom
        // account with WebSockets disabled must still use the explicit provider
        // table so Codex cannot fall back to a stale top-level openai_base_url.
        let account = CodexAccount::new_api_key(
            "relay-missing-wire".to_string(),
            "relay-missing-wire@example.com".to_string(),
            "sk-test".to_string(),
            CodexApiProviderMode::Custom,
            Some("https://relay-missing-wire.example/v1".to_string()),
            Some("relay_missing_wire".to_string()),
            Some("Relay Missing Wire".to_string()),
            Vec::new(),
        );
...
"#,
    ...
    fn remote_api_key_imagegen_does_not_disable_hosted_chat_tool() {
        ...
        let provider_config = resolve_api_provider_config(
            Some("https://relay.example.cn/v1"),
            Some(CodexApiProviderMode::Custom),
            Some("custom_relay"),
            Some("Custom Relay"),
        )
...
"#,
    ...
    fn api_key_bundle_enables_imagegen_when_catalog_contains_image_model() {
        ...
        assert!(content.contains("respect_system_proxy = true"));
