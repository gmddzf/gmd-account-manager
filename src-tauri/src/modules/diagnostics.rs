...
const SENTRY_CLIENT: &str = "gmd-account-manager/1.0";
const PROFILE_ENV: &str = "GMD_ACCOUNT_MANAGER_PROFILE";
...
fn build_sentry_event(
    ...
    let mut event = json!({
        ...
        "platform": "rust",
        "level": level,
        "release": format!("gmd-account-manager@{}", env!("CARGO_PKG_VERSION")),
...
fn current_dsn() -> Option<String> {
    let runtime = std::env::var("GMD_SENTRY_DSN")
        .ok()
        .or_else(|| option_env!("GMD_SENTRY_DSN").map(ToString::to_string))
        .unwrap_or_default();
...
fn should_send(level: &str, message: &str, source: Option<&str>, phase: Option<&str>) -> bool {
    let env_disabled = std::env::var("GMD_DISABLE_ERROR_REPORTING")
        .ok()
        .map(|value| {
