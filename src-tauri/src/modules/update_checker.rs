...
fn get_data_dir() -> Result<std::path::PathBuf, String> {
    dirs::data_local_dir()
        .map(|d| d.join("gmd-account-manager"))
...
/// Customer builds must not surface inherited partner, referral, or discount
/// copy through bundled or remotely supplied release notes.
fn is_promotional_release_note(content: &str) -> bool {
    let legacy_relay_brand = ["apikey", ".fun"].concat();
    let normalized = content.to_lowercase();
    [
        legacy_relay_brand.as_str(),
        "affiliate",
        "aff=",
        "invitecode",
        "referral",
...
fn sanitize_release_note_content(content: &str) -> Option<String> {
    if is_promotional_release_note(content) {
        return None;
    }

    let replacements = [
        ("Antigravity Cockpit Tools", "GMD 账号管理"),
        ("Cockpit Tools", "GMD 账号管理"),
        ("Cockpit API", "GMD API"),
    ];
    let rebranded = replacements.iter().fold(content.to_string(), |value, entry| {
        replace_ascii_case_insensitive(&value, entry.0, entry.1)
    });
    Some(replace_ascii_case_insensitive(&rebranded, "Cockpit", "GMD"))
}
...
fn parse_release_history_markdown(markdown: &str, limit: usize) -> Vec<ReleaseHistoryItem> {
    ...
    for raw_line in normalized.lines() {
        ...
        let Some(content) = sanitize_release_note_content(content) else {
            continue;
        };
        if content.is_empty() {
            continue;
        ...
        match current_section {
            ReleaseHistorySection::Added => release.added.push(content.clone()),
            ReleaseHistorySection::Changed => release.changed.push(content.clone()),
            ReleaseHistorySection::Fixed => release.fixed.push(content.clone()),
            ReleaseHistorySection::Removed => release.removed.push(content.clone()),
...
"#;
    ...
    #[test]
    fn test_release_history_filters_partner_promotions_and_rebrands_legacy_copy() {
        let legacy_brand = ["APIKEY", ".FUN"].concat();
        let input = format!(
            r#"
## [1.0.0] - 2026-08-15
### Added
- GMD relay improvements
- {legacy_brand} partner discount and referral invitecode
...
"#
        ...
        assert_eq!(
            result[0].added,
            vec![
                "GMD relay improvements".to_string(),
                "GMD 账号管理 was renamed".to_string(),
                "GMD dashboard was renamed".to_string(),
            ]
        );
    }
}
