...
fn is_supported_scheme(scheme: &str) -> bool {
    matches!(
        scheme,
        "gmd-account-manager" | "gmdaccountmanager" | "cockpit-tools" | "cockpittools"
    )
}
...
pub fn handle_external_import_args<R: Runtime>(
    ...
    for (i, arg) in args.iter().enumerate() {
        ...
        let candidate_scheme = candidate
            .split_once("://")
            .map(|(scheme, _)| scheme.to_ascii_lowercase());
        let candidate_is_deep_link = candidate_scheme
            .as_deref()
            .map(is_supported_scheme)
            .unwrap_or(false);
...
mod tests {
    ...
    fn parse_basic_import_link() {
        let raw = "gmd-account-manager://import?provider=codex&token=abc123";
    ...
    #[test]
    fn keeps_legacy_import_scheme_compatible() {
        let raw = "cockpit-tools://import?provider=codex&token=abc123";
        let payload = parse_external_import_url(raw).expect("legacy payload");
        assert_eq!(payload.provider_id, "codex");
        assert_eq!(payload.token, "abc123");
    }
