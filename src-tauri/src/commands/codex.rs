...
fn codex_model_provider_new_api_billing_url(
    ...
    // The customer-facing relay field is normalized to `/v1` for model
    // calls, while New API's dashboard endpoints live at the host root.
    if base_path.eq_ignore_ascii_case("/v1") {
        base_path.clear();
    }
...
fn json_i64_any_at(value: &serde_json::Value, paths: &[&[&str]]) -> Option<i64> {
    paths.iter().find_map(|path| json_i64_at(value, path))
}
...
fn summarize_new_api_model_provider_usage(
    ...
    // New API deployments have returned request counters at more than one
    // response level over time. Read only explicit fields and leave the
    // counters empty when the server does not expose them.
    let today_requests = token_data
        .and_then(|value| {
            json_i64_any_at(
                value,
                &[
...
mod tests {
    ...
    #[test]
    fn new_api_billing_urls_ignore_optional_v1_path() {
        assert_eq!(
            codex_model_provider_new_api_billing_url(
                "https://api.gmd.ink/v1",
                "dashboard/billing/subscription",
            )
            .expect("valid URL"),
            "https://api.gmd.ink/dashboard/billing/subscription"
        );
        assert_eq!(
            codex_model_provider_new_api_billing_url(
                "https://api.gmd.ink",
            ...
            .expect("valid URL"),
            "https://api.gmd.ink/dashboard/billing/usage"
        );
    }
    ...
    fn new_api_usage_counters_accept_explicit_server_fields() {
        ...
        assert!(summary
            .details
            .iter()
            .any(|item| item.key == "todayRequests" && item.value == "7"));
        assert!(summary
            .details
            .iter()
            .any(|item| item.key == "totalRequests" && item.value == "42"));
    }

    #[test]
    fn new_api_money_ignores_raw_token_quota_units() {
        let subscription = serde_json::json!({
            "hard_limit_usd": 100.0,
        });
        let usage = serde_json::json!({
        ...
        let token_usage = serde_json::json!({
            "data": {
                "total_used": 6_250_000.0,
                "total_available": 43_750_000.0,
