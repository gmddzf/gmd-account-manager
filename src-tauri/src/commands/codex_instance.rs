...
/// 等待自动切号后新 Codex 进程真正可接受连接，再把启动流程报告给前端。
///
/// 以前这里只等待 `start_codex_*` 返回。Windows/macOS 上进程已经存在时，
/// Codex 的 WebView/CDP 和本地 provider gateway 仍可能尚未就绪，用户随后
/// 立刻发起请求就会看到连续的 reconnect 5/5。CDP 端口可用时以 `/json/version`
/// 为准；没有注入端口时保留一个短暂的启动缓冲，不改变账号或路由配置。
async fn wait_for_codex_startup_ready(pid: u32, cdp_port: Option<u16>) {
    const MAX_WAIT: Duration = Duration::from_secs(8);
    const POLL: Duration = Duration::from_millis(150);
    const NO_CDP_GRACE: Duration = Duration::from_millis(1200);

    let started = Instant::now();
    if let Some(port) = cdp_port {
        let client = Client::builder()
            // The readiness probe is loopback-only.  Never send it through a
            // customer HTTP proxy, especially while the proxy feature is being
            // enabled for the Codex child process.
            .no_proxy()
        ...
        loop {
            if !modules::process::is_pid_running(pid) {
                modules::logger::log_warn(&format!(
                    "[Codex Start] Codex 在就绪检查期间退出: pid={}, port={}",
                    pid, port
                ));
                return;
            }
            let ready = match client.as_ref() {
                Some(client) => client
                    .get(&url)
            ...
            if ready {
                modules::logger::log_info(&format!(
                    "[Codex Start] Codex 就绪检查通过: pid={}, port={}, elapsed_ms={}",
                    pid,
                    port,
            ...
            if started.elapsed() >= MAX_WAIT {
                modules::logger::log_warn(&format!(
                    "[Codex Start] Codex 就绪检查超时，继续返回: pid={}, port={}, elapsed_ms={}",
                    pid,
                    port,
                    started.elapsed().as_millis()
                ));
    ...
    if !modules::process::is_pid_running(pid) {
        modules::logger::log_warn(&format!(
            "[Codex Start] Codex 在无 CDP 启动缓冲前已退出: pid={}",
            pid
        ));
        return;
    }
    tokio::time::sleep(NO_CDP_GRACE).await;
    modules::logger::log_info(&format!(
        "[Codex Start] Codex 无 CDP 端口，完成启动缓冲: pid={}, elapsed_ms={}",
        pid,
        started.elapsed().as_millis()
    ));
}
...
mod tests {
    ...
    fn launch_command_preview_does_not_require_an_initialized_profile() {
        ...
        let command = build_launch_command_preview(&context, Some("gpt-selected"))
        ...
        assert!(command.contains("gpt-selected"));
        assert!(
            command.rfind("gpt-selected").unwrap() > command.rfind("gpt-test").unwrap(),
            "the selected launch model must be the final model override"
        );
    }

    #[test]
    fn cli_launch_model_normalization_accepts_simple_ids_and_blank_means_no_override() {
        assert_eq!(
            normalize_cli_model(Some("  gpt-5.6-sol  ".to_string())).unwrap(),
            Some("gpt-5.6-sol".to_string())
        );
        assert_eq!(normalize_cli_model(Some("   ".to_string())).unwrap(), None);
        assert_eq!(normalize_cli_model(None).unwrap(), None);
    }

    #[test]
    fn cli_launch_model_normalization_rejects_unsafe_or_oversized_values() {
        for invalid in ["gpt selected", "gpt\tselected", "gpt\nselected"] {
            assert!(
                normalize_cli_model(Some(invalid.to_string())).is_err(),
                "model should be rejected: {invalid:?}"
            );
        }

        assert!(normalize_cli_model(Some("m".repeat(201))).is_err());
    }
...
fn normalize_cli_model(model: Option<String>) -> Result<Option<String>, String> {
...
fn build_launch_command_text(
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        if let Some(model) = model {
            codex_cmd.push_str(" --model ");
            codex_cmd.push_str(&posix_shell_quote(model));
        }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(model) = model {
            codex_cmd.push_str(" --model ");
            codex_cmd.push_str(&powershell_quote(model));
        }
...
fn build_launch_command_preview(
    context: &CodexLaunchContext,
    model: Option<&str>,
) -> Result<String, String> {
    build_launch_command_text(context, "codex", None, model)
}
...
async fn codex_start_instance_internal(
    ...
    if instance_id == DEFAULT_INSTANCE_ID {
        ...
        if default_settings.launch_mode == InstanceLaunchMode::Cli {
            ...
            let _ = build_launch_command(&context, None)?;
            let _ = modules::codex_instance::update_default_pid(None)?;
            modules::logger::log_info(&format!(
        ...
        wait_for_codex_startup_ready(pid, injection_plan.port).await;
...
pub async fn codex_preview_instance_launch_command(
    ...
    let launch_command = launch_command
        ...
        .map(Ok)
        .unwrap_or_else(|| build_launch_command_preview(&context, model.as_deref()))?;
...
pub async fn codex_get_instance_launch_command(
    ...
    let model = normalize_cli_model(model)?;
    let launch_command = build_launch_command(&context, model.as_deref())?;
...
pub async fn codex_execute_instance_launch_command(
    ...
    let context = resolve_instance_launch_context(&instance_id)?;
    let model = normalize_cli_model(model)?;
    let command = build_launch_command(&context, model.as_deref())?;
    let terminal = resolve_codex_launch_terminal(terminal);
