...
fn should_skip_sidecar_build(output: &Path) -> bool {
    std::env::var("GMD_SKIP_CLIPROXY_BUILD").ok().as_deref() == Some("1") && output.exists()
}
...
fn build_go_sidecar(
    ...
    let output = output_dir.join(format!("gmd-cliproxy-{rust_target}{extension}"));
    ...
    let status = Command::new("go")
        ...
        .arg(".")
        .status()
        .expect("failed to start go build for gmd-cliproxy");

    if !status.success() {
        panic!("go build for gmd-cliproxy failed with status: {status}");
    }
...
fn build_macos_universal_sidecar(sidecar_dir: &Path, output_dir: &Path) {
    let output = output_dir.join("gmd-cliproxy-universal-apple-darwin");
    if should_skip_sidecar_build(&output) {
        return;
    }
    ...
    let status = Command::new("lipo")
        ...
        .arg(&output)
        .status()
        .expect("failed to start lipo for gmd-cliproxy universal sidecar");

    if !status.success() {
        panic!("lipo for gmd-cliproxy universal sidecar failed with status: {status}");
    }
}
...
fn build_cockpit_cliproxy_sidecar() {
    ...
    println!("cargo:rerun-if-env-changed=GMD_SKIP_CLIPROXY_BUILD");
    emit_sidecar_rerun_inputs(&sidecar_dir);
    std::fs::create_dir_all(&output_dir).expect("failed to create gmd-cliproxy bin dir");
